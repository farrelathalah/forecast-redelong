...
def metrics_for(df: pd.DataFrame, rain_col: str | None, source_col: str | None) -> dict:
    ...
    if source_col and source_col in df.columns:
        out["sources"] = str(df[source_col].dropna().astype(str).nunique())

    if rain_col and rain_col in df.columns and not df.empty:
...
<body>
  ...
  <header class="hero" id="top">
    <div class="hero-inner">
      <div class="hero-copy">
        <div class="kicker">Ensemble Rainfall Forecast</div>
        <h1><span class="gradient-text">Forecast hujan</span> untuk PLTA Redelong.</h1>
        <p class="subtitle">
          Portal prakiraan hujan berbasis multi-source forecast untuk titik PLTA Redelong,
      ...
      <aside class="hero-panel">
        <div class="metric"><div class="label">Titik Catchment</div><div class="value">{len(locations)}</div></div>
        <div class="metric"><div class="label">Sumber Aktif</div><div class="value">{stats["sources"]}</div></div>
        <div class="metric"><div class="label">Baris Forecast</div><div class="value">{stats["rows"]}</div></div>
  ...
  <section id="ringkasan">
    <div class="section-inner">
      <div class="section-head reveal">
        <h2>Ringkasan hujan.</h2>
        <p>Ringkasan ini dihitung langsung dari output forecast gabungan, bukan dari dashboard kosong bawaan template.</p>
      </div>
      <div class="cards">
        <article class="info-card reveal"><p class="eyebrow">Rain Max</p><div class="big-number">{stats["rain_max"]} mm</div><p>Nilai maksimum hujan dari output gabungan.</p></article>
        <article class="info-card reveal"><p class="eyebrow">Rain P90</p><div class="big-number">{stats["rain_p90"]} mm</div><p>Persentil 90 untuk membaca skenario atas.</p></article>
        <article class="info-card reveal"><p class="eyebrow">Rain Mean</p><div class="big-number">{stats["rain_mean"]} mm</div><p>Rata-rata nilai hujan pada seluruh titik dan sumber.</p></article>
        <article class="info-card reveal"><p class="eyebrow">Status</p><div class="big-number" style="font-size:28px">Prototype</div><p>Validasi observasi aktual tetap diperlukan.</p></article>
      </div>
    </div>
  </section>
  ...
  <section id="lokasi">
    <div class="section-inner">
      <div class="section-head reveal">
        ...
        <p>Dashboard utama per titik sekarang memakai halaman hujan khusus, bukan halaman template yang kosong.</p>
      </div>
      <div class="locations">
        {''.join(cards)}
      </div>
...
def make_location_dashboard(slug, loc, loc_df, rain_col, source_col, time_cols, temp_col, wind_col):
    ...
    chart = make_bar_chart(loc_df, rain_col, time_cols)
...
<body>
  ...
  <header class="hero">
    <div class="hero-inner">
      ...
      <aside class="hero-panel">
        <div class="metric"><div class="label">Rain Max</div><div class="value">{stats["rain_max"]} mm</div></div>
        <div class="metric"><div class="label">Rain P90</div><div class="value">{stats["rain_p90"]} mm</div></div>
        <div class="metric"><div class="label">Rain Mean</div><div class="value">{stats["rain_mean"]} mm</div></div>
        <div class="metric"><div class="label">Sumber</div><div class="value">{stats["sources"]}</div></div>
      </aside>
    </div>
  ...
  <section id="grafik">
    <div class="section-inner">
      <div class="section-head reveal">
        <h2>Grafik hujan.</h2>
        <p>Grafik ini menampilkan ringkasan nilai hujan dari forecast yang tersedia untuk titik ini.</p>
      </div>
...
<body>
  ...
  <section style="padding-top:130px">
    <div class="section-inner">
      <div class="section-head reveal">
        <h2>Peta hujan.</h2>
        <p>Peta ini menampilkan titik PLTA Redelong dan GPM catchment dengan ringkasan hujan dari output forecast.</p>
      </div>
  ...
  <script>
    ...
    points.forEach(p => {{
      ...
      marker.bindPopup(`
        ...
        Rain Max: ${{p.rain_max}} mm<br>
        Rain P90: ${{p.rain_p90}} mm<br>
        Rain Mean: ${{p.rain_mean}} mm<br>
        Sumber: ${{p.sources}}<br>
