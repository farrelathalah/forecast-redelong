...
CATCHMENT = ROOT / "data" / "sites" / "pltm_besai_kemu" / "besai_kemu_catchment.geojson"
...
def site_card(slug: str, site: dict) -> str:
    ...
    verified = site["site_status"] == "verified_from_project_package"
    engineering = site["site_status"].startswith("engineering_document_reference")
    status_class = "ready" if verified else "engineering" if engineering else "provisional"
    status_text = "Data proyek tersedia" if verified else "Referensi engineering" if engineering else "Sumber publik provisional"
...
def page_html(payload: dict) -> str:
    ...
    catchment_json = CATCHMENT.read_text(encoding="utf-8")
...
</style><style>.status.engineering{{background:rgba(34,211,238,.13);color:#67e8f9}}</style></head><body><div id="map" aria-label="Globe cuaca Forecast Site"></div><div class="top"><a class="brand" data-fr-brand="true" href="index.html"><span class="mark">FR</span><span><b>Forecast Site</b><small>Cuaca dan hidrologi site</small></span></a><a class="home" href="index.html">Kembali ke portal</a></div>
<aside class="panel"><div class="eyebrow">Globe Forecast Site</div><h1>Seluruh site dalam satu globe.</h1><p class="intro">Redelong dan Besai Kemu ditampilkan sejak awal. Outline Besai Kemu adalah batas indikatif dari gambar FS, bukan vektor survei atau batas legal.</p><div class="sites">{cards}</div></aside>
<script>const SITES={site_json};const BESAI_CATCHMENT={catchment_json};if(typeof maplibregl==='undefined')throw new Error('MapLibre unavailable');const map=new maplibregl.Map({{container:'map',center:[104,-1],zoom:2.7,style:{{version:8,projection:{{type:'globe'}},sources:{{satellite:{{type:'raster',tiles:['https://tiles.maps.eox.at/wmts/1.0.0/s2cloudless-2020_3857/default/g/{{z}}/{{y}}/{{x}}.jpg'],tileSize:256,attribution:'Cloudless imagery, EOX'}}}},layers:[{{id:'satellite',type:'raster',source:'satellite'}}],sky:{{'atmosphere-blend':['interpolate',['linear'],['zoom'],0,1,7,0]}}}}}});map.addControl(new maplibregl.NavigationControl({{visualizePitch:true}}),'top-right');map.on('load',()=>{{map.addSource('besai-catchment',{{type:'geojson',data:BESAI_CATCHMENT}});map.addLayer({{id:'besai-catchment-fill',type:'fill',source:'besai-catchment',paint:{{'fill-color':'#fbbf24','fill-opacity':0.14}}}});map.addLayer({{id:'besai-catchment-outline',type:'line',source:'besai-catchment',paint:{{'line-color':'#fde68a','line-width':['interpolate',['linear'],['zoom'],2,1,8,2.5],'line-dasharray':[3,2]}}}});map.addSource('sites',{{type:'geojson',data:{{type:'FeatureCollection',features:SITES.map(s=>({{type:'Feature',properties:s,geometry:{{type:'Point',coordinates:[s.longitude,s.latitude]}}}}))}}}});map.addLayer({{id:'site-points',type:'circle',source:'sites',paint:{{'circle-radius':['interpolate',['linear'],['zoom'],2,5,8,10],'circle-color':['case',['==',['get','status'],'verified_from_project_package'],'#34d399','#fbbf24'],'circle-stroke-color':'#04111f','circle-stroke-width':3}}}});map.on('click','site-points',e=>{{if(!e.features)return;const f=e.features[0],p=f.properties;new maplibregl.Popup({{offset:12}}).setLngLat(f.geometry.coordinates).setHTML(`<b>${{p.name}}</b><br>${{p.plant_type}}, ${{p.province}}<br>${{p.status.includes('provisional')?'Data provisional':'Data proyek tersedia'}}`).addTo(map);}});map.on('mouseenter','site-points',()=>map.getCanvas().style.cursor='pointer');map.on('mouseleave','site-points',()=>map.getCanvas().style.cursor='');}});document.querySelectorAll('[data-site]').forEach(card=>card.addEventListener('mouseenter',()=>{{const site=SITES.find(s=>s.slug===card.dataset.site);map.flyTo({{center:[site.longitude,site.latitude],zoom:site.slug==='pltm_besai_kemu'?8:7,pitch:38,duration:1400}});}}));</script></body></html>'''
...
def build(outputs: Path) -> None:
    ...
    (outputs / "besai_kemu_catchment.geojson").write_text(
        CATCHMENT.read_text(encoding="utf-8"), encoding="utf-8"
    )
